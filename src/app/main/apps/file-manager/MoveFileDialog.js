...
function MoveFileDialog() {
	...
	const handleMoveFile = () => {
		...
		console.log('files?????????????????????fileType', fileType)
		console.log('files?????????????????????fileId', fileId)
		console.log('files?????????????????????relative_path', relative_path)
		...
		// }
		console.log('files?????????????????????values', values)
		apiCall(
			apiUrl,
			values,
			res => {
				console.log('files?????????????????????111111', res)
				const userInfo = decodeDataFromToken();
				const cid = userInfo.extra?.profile?.company;
				if (fileType == 'folder') {
					console.log('files?????????????????????22222')
				...
				if (folderPath.length > 1) {
					...
					dispatch(Actions.folderDetail(cid, values));
				}
				if (fileType != 'folder') {
					console.log('files?????????????????????44444')
					if (fileType == 'photo') {
						console.log('files?????????????????????55555')
						dispatch(Actions.getPhotos(cid));
					} else if (fileType == 'video') {
						console.log('files?????????????????????666666')
						dispatch(Actions.getVideos(cid));
					} else {
						console.log('files?????????????????????777777')
